...
RegisterNUICallback('createNewCharacter', function(data, cb)
    ...
    -- Gender kontrolü düzeltildi
    if cData.gender == "male" or cData.gender == Lang:t("ui.male") then
        cData.gender = 0
    elseif cData.gender == "female" or cData.gender == Lang:t("ui.female") then
        cData.gender = 1
    end
    
    -- Tarih validasyonu
    local day = tonumber(cData.birthdate:sub(9, 10))
    local month = tonumber(cData.birthdate:sub(6, 7))
    ...
    if year < (currentYear - 100) or year > (currentYear - 18) then
        isValidDate = false
        print("^1[MULTICHARACTER ERROR]^7 Geçersiz yıl: " .. year .. " (18-100 yaş arası olmalı)")
    end
    
    -- Ay kontrolü (1-12 arası)
    if month < 1 or month > 12 then
        isValidDate = false
        print("^1[MULTICHARACTER ERROR]^7 Geçersiz ay: " .. month .. " (1-12 arası olmalı)")
    end
    
    -- Gün kontrolü (aya göre)
    ...
    if day < 1 or day > daysInMonth[month] then
        isValidDate = false
        print("^1[MULTICHARACTER ERROR]^7 Geçersiz gün: " .. day .. " (Ay " .. month .. " için 1-" .. daysInMonth[month] .. " arası olmalı)")
    end
    
    if isValidDate then
        print("^2[MULTICHARACTER]^7 Karakter oluşturuluyor - Cinsiyet: " .. cData.gender .. " (0=Erkek, 1=Kadın)")
        TriggerServerEvent('hb_multicharacter:server:createCharacter', cData)
        Wait(500)
        cb("ok")
    else
        print("^1[MULTICHARACTER ERROR]^7 Geçersiz tarih, karakter oluşturulamadı!")
        DoScreenFadeIn(150)
        cb("error")
    end
end)
