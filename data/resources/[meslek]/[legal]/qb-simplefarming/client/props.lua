...
local RentMan = Rent.coords
local HastaneMan = Hastane.coords
...
CreateThread(function()
	...
	pedfarmer5 = CreatePed(1, 0x94562DD7, RentMan, false, true)
	pedfarmer6 = CreatePed(1, 0xD47303AC, HastaneMan, false, true)
	...
	SetEntityInvincible(pedfarmer6, true)
    SetBlockingOfNonTemporaryEvents(pedfarmer6, true)
    FreezeEntityPosition(pedfarmer6, true)
  end)
  ...
  QBCore.Functions.TriggerCallback('qb-simplefarming:weapon_knife', function(hasItem)
    if hasItem then
      if DoesEntityExist(pedcow2) then
        ExecuteCommand( "e mekanik4" )
        QBCore.Functions.Progressbar('killing_cow', Config.Alerts['killing_cow'], 12000, false, true, {
          disableMovement = true,
          disableCarMovement = true,
          disableMouse = false,
          disableCombat = true,
