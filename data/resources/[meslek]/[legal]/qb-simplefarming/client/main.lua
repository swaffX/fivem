...
RegisterNetEvent('qb-getcowbucket', function()
  ...
  AttatityToEntity(milkprop, PlayerPedId(), GetPedBoneIndex(PlayerPedId(), 60309), 0.12, 0, 0.30, -145.0, 100.0, 0.0, true, true, false, true, 1, true)
  TaskPlayAnim(PlayerPedId(), "anim@heists@box_carry@", "idle", 2.0, 2.0, 2500, 51, 0, false, false, false)
  Citizen.Wait(2500)
  Detatity(milkprop, 1, true)
...
RegisterNetEvent('qb-simplefarming:pumpkinfarming', function()
  ...
  QBCore.Functions.Progressbar("picking_pumpkins", Config.Alerts['picking_pumpkins'], 3000, false, true, {
      ...
      AttatityToEntity(pumpkinprop, PlayerPedId(), GetPedBoneIndex(PlayerPedId(), 60309), 0.12, 0, 0.30, -145.0, 100.0, 0.0, true, true, false, true, 1, true)
      TaskPlayAnim(PlayerPedId(), "anim@heists@box_carry@", "idle", 2.0, 2.0, 2500, 51, 0, false, false, false)
      Citizen.Wait(3500)
      Detatity(pumpkinprop, 1, true)
...
RegisterNetEvent('qb-simplefarming:petpiggy', function()
  ExecuteCommand( "e mekanik3" )
  QBCore.Functions.Progressbar("pet_pig", "Domuz Seviliyor", 5000, false, true, { -- 5 Seconds
      disableMovement = true,
      disableCarMovement = true,
      disableMouse = false,
      disableCombat = true,
...
RegisterNetEvent('qb-simplefarming:feedpig', function ()
  QBCore.Functions.TriggerCallback('qb-simplefarming:soybeancheck', function(soybeans)
    if soybeans then
	  ExecuteCommand( "e mekanik3" )
	  QBCore.Functions.Progressbar("feeding_pig", "Domuz Besleniyor", 5000, false, true, { -- 5 Seconds
		  disableMovement = true,
		  disableCarMovement = true,
		  disableMouse = false,
		  disableCombat = true,
		  ...
		  Wait(2000)
		  QBCore.Functions.Notify("Şu an daha da rahatsın")
	  end, function() 
		  ClearPedTasks(PlayerPedId())
		  QBCore.Functions.Notify("İşlem iptal edildi.", "error")
	  end)
    elseif not soybeans then
      QBCore.Functions.Notify("Bunun için soya fasulyesine sahip olmalısın", "error", 3000)
    end
	end)
end)
...
RegisterNetEvent('qb-simplefarming:menuprocessor', function(data)
  local Processor = {
    ...
    {
        header = 'Acı Biber Yap',
        params = {
            event = 'qb-simplefarming:makingchillysauce',
        }
    },
    {
        header = 'Domates Pastası Yap',
        params = {
...
RegisterNetEvent('qb-simplefarming:menufcow', function()
  local MeatCooking = {
    ...
    {
        header = 'Pastırma Pişir',
        params = {
            event = 'qb-simplefarming:baconprocessing',
        }
    },
    {
        header = 'Domuz Eti Pişir',
        params = {
