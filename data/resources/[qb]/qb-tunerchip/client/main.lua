...
RegisterNUICallback('save', function(data, cb)
    QBCore.Functions.TriggerCallback('qb-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            ...
            setVehData(veh, data)
            QBCore.Functions.Notify('TunerChip v1.05: Araç Ayarlı!', 'error')
...
RegisterNetEvent('qb-tunerchip:client:TuneStatus', function()
    ...
    if vehModel ~= 0 then
        QBCore.Functions.TriggerCallback('qb-tunerchip:server:GetStatus', function(status)
            if status then
                QBCore.Functions.Notify('Bu Araç Ayarlandı', 'success')
            else
                QBCore.Functions.Notify('Bu Araç Ayarlanmadı', 'error')
            end
        end, plate)
    else
        QBCore.Functions.Notify('Yakınlarda Araç Yok', 'error')
    end
end)
...
RegisterNUICallback('reset', function(_, cb)
    ...
    local veh = GetVehiclePedIsUsing(ped)
    resetVeh(veh)
    QBCore.Functions.Notify('TunerChip v1.05: Araç sıfırlandı!', 'error')
    cb("ok")
end)
...
RegisterNetEvent('qb-tunerchip:client:openChip', function()
    ...
    if inVehicle then
        QBCore.Functions.Progressbar("connect_laptop", "TunerChip v1.05: Araç sıfırlandı!", 2000, false, true, {
        ...
        QBCore.Functions.Notify("Bir Araçta Değilsiniz", "error")
    end
end)
