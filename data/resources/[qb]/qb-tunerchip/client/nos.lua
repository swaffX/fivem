    ...
    if not NitrousActivated then
        if IsInVehicle and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
            if GetPedInVehicleSeat(veh, -1) == ped then
                QBCore.Functions.Progressbar("use_nos", "Turbo Takılıyor...", 1000, false, true, {
                ...
                QBCore.Functions.Notify("Bunu bu koltuktan yapamazsın!", "error")
            end
        else
            QBCore.Functions.Notify('Arabada değilsin', 'error')
        end
    else
        QBCore.Functions.Notify('Zaten Turbo Aktif', 'error')
    end
end)
        ...
        if IsInVehicle then
            ...
            if VehicleNitrous[Plate] ~= nil then
                if VehicleNitrous[Plate].hasnitro then
                    if IsControlJustPressed(0, 61) and GetPedInVehicleSeat(CurrentVehicle, -1) == PlayerPedId() then
                    ...
                    if IsControlJustReleased(0, 61) and GetPedInVehicleSeat(CurrentVehicle, -1) == PlayerPedId() then
